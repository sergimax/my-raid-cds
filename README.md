...
10. **Gear upgrade hints** — When a character has imported gear for a spec, toggle cells may show a background tint for upgrades in that raid (brighter = more slots). **Amber** tint = missing **BiS list** targets (exact item or same-name normal/heroic variant) when a list is selected. **Blue** tint (intensity by slot count) = other **stat-filtered** ilvl upgrades (equip rules + spec stat weights; e.g. spirit leather no longer suggested for Enhancement). Hints are hidden once the toggle is on (dungeon marked complete for that character). Dungeon **name** and **ilvl** columns stay plain; ilvl-tier colors remain on item links only. Hints are **per spec** (main and off each use their own gear and BiS list); dual-spec cells split left (main) / right (off). Hover for per-spec tooltips — compact summary line, scrollable boss-grouped BiS loot (missing items only; exact + normal variants as separate sections), and tier set tokens on ICC / ToC token rows when applicable. Custom dungeons without bundled loot use peak ilvl only. Boss and raid labels follow the UI locale (EN/RU). See [docs/roadmap.md](docs/roadmap.md) for possible follow-ups (trinket procs, finer tuning).
