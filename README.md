...
- **Character pick** — Copy a raid roster of characters still missing CD for visible (filtered) rows. Open via toolbar **Character pick** (EN) / **Подбор персонажа** (RU). The panel uses a bordered filter grid: **min GS**, **role** (WoW LFG icons in an equal 2×2 grid), **character specs** (with **Select all** / **Clear all**), and **raids** (table search, chips with raid icons). **Reset all filters** in the panel header clears specs, roles, and min GS. At viewport widths ≥1600px, signup result lines sit to the right of the filter grid (scroll inside the block). Per-raid lines include a **Copy** button; optional role and minimum GS filters disable specs that do not match.
