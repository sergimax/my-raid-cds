...
10. **Gear upgrade hints** — When a character has imported gear for a spec, toggle cells for a dungeon may show a subtle background tint if upgrades exist in that raid’s item-level tier (brighter tint = more upgradeable slots). Hints are computed **per spec** (main and off each use their own gear and BiS list). When both specs have hints, the cell tint is split left (main) / right (off). Hover the toggle for per-spec summaries of upgradeable slots (and BiS gaps when a list is selected). Loot is filtered by the character’s **class** and that spec (e.g. priests see wand upgrades, not crossbows; Fury warriors may get 2H off-hand suggestions). On ICC / ToC rows that drop tier tokens, the tooltip also lists which sanctification or regalia/triumph tokens are still needed for BiS tier set pieces — token type, quantity, and slots. Hints use bundled raid loot when the dungeon resolves to a template `raidKey` (by saved key or localized raid name); with a BiS list selected for that spec, only BiS-relevant loot and set upgrades count.
...
```
src/
├── components/       # app-header, tracker-layout, raid-tracker-main, tracker-toolbar-panel, character-form, character-edit-dialog (+ character-spec-gear-import-section, character-edit-spec-gear), character-spec-gear-fields, spec-option-label, dungeon-form, dungeon-edit-dialog, dungeon-customization-fields, export-panel, bis-lists-panel, gear-hint-tooltip, wow-item-link (+ item-link-styles.ts), stored-gear-item-line, item-tooltip-locale-toggle, tracker-controls, …
