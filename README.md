...
- **Export** — Copy a raid roster of characters still missing CD for visible (filtered) rows. The export panel uses a bordered filter grid: **min GS** (slider with large value readout), **role** (tank / healer / melee / ranged with WoW LFG icons in a 2×2 layout), **character specs** (main/off checkboxes per row; scrolls when there are many characters), and **raids** (search summary + chips for matching dungeons). Optional **role filter** (all on by default): unchecked roles disable matching spec checkboxes and omit them from lines. Optional **minimum GS** slider (`≥ 5.6`, etc.): when enabled, specs below the threshold are disabled the same way (stored toggles restore when filters are cleared).
