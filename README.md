...
10. **Gear upgrade hints** — When a character has imported gear for a spec, toggle cells may show a background tint for upgrades in that raid (brighter = more slots). **Amber** tint = missing **BiS list** targets that drop here (when a list is selected for that spec; takes priority over ilvl tint). **Dungeon ilvl-tier color** tint = raid loot or ilvl upgrades where equipped gear is below what the raid drops (always evaluated alongside BiS). Hints are **per spec** (main and off each use their own gear and BiS list); dual-spec cells split left (main) / right (off). Hover for per-spec summaries — BiS gaps and ilvl upgrades listed separately when both apply. Loot is filtered by **class** and spec. On ICC / ToC token rows, the tooltip also lists tier set tokens still needed for BiS set pieces. Custom dungeons without bundled loot use peak ilvl only.
