...
| `npm run build:wow-data` | Regenerate bundled item names, gear slots, equip metadata, item stats (sparse WowSims indices), raid loot, item drop sources (boss + raid per item id), and tier sets from `scripts/wowsims-db.json` (use `--skip-ru` to skip WoWRoad Russian name fetch) |
...
| `npm run generate:bis-sources` | Generate community BiS preset TypeScript from `scripts/bis-list-sources.md` (Unholy DK, Enhancement Shaman, Feral Druid, Restoration Druid; writes four preset files only) |
...
8. **BiS lists** — **BiS lists** in the toolbar opens the same outlined panel shell (title, layout hint, close ✕) to pick class and spec (with class and talent icons in the dropdowns), browse built-in presets (read-only; **Titans** lists for most WotLK specs, plus community-source lists for Unholy Death Knight, Enhancement Shaman, Feral Druid, and Restoration Druid from Warmane / Circle / Icy-Veins guides), and create or edit local presets with item IDs per gear slot. Specs without a built-in list (e.g. Warrior Arms) still get the full slot editor and **Save list** form so you can build a custom list from scratch. Item names in slot rows use the same colored link styling as stored gear; when bundled drop data exists, a muted caption below shows raid + boss (export-style abbreviation, localized — e.g. `ICC25H · Lord Marrowgar`, `РС25хм · Халион`). Slots without a raid drop (tier tokens, crafted, etc.) show the item name only. Confirm each slot with the check icon; **Save list** stores a local preset under a custom name (built-in lists can be copied this way). Delete local presets from the preset chips. Slot validation rejects items the selected class/spec cannot equip (armor type, weapons, ranged slot). The selected preset for each class/spec drives upgrade-hint filtering for characters with that spec.
...
10. **Gear upgrade hints** — When a character has imported gear for a spec, toggle cells may show a background tint for upgrades in that raid (brighter = more slots). **Amber** tint = missing **BiS list** targets that drop here (when a list is selected for that spec; takes priority over ilvl tint). **Dungeon ilvl-tier color** tint = *possible* higher-ilvl raid loot or ilvl gaps (equipable for class/spec, filtered by spec-relevant primary stats on the **ilvl** track — e.g. caster mail is omitted for Enhancement). Hints are **per spec** (main and off each use their own gear and BiS list); dual-spec cells split left (main) / right (off). Hover for per-spec summaries with talent icons — BiS gaps and possible ilvl upgrades listed separately when both apply; when drop data matches the dungeon row, **BiS loot in this raid** groups items by boss with item links. On ICC / ToC token rows, the tooltip also lists tier set tokens still needed for BiS set pieces (compact lines). Custom dungeons without bundled loot use peak ilvl only. Boss and raid labels follow the UI locale (EN/RU). See [docs/roadmap.md](docs/roadmap.md) for possible follow-ups (trinket procs, finer tuning).
...
├── data/             # raid-names.ts, raid-boss-names.ts, class-specs.ts, dungeon-list.ts, create-template-dungeon.ts, dungeons.ts, bis-presets/, item-drop-sources.ts, gear-slot-names.ts, raid-loot.ts, wotlk-item-levels.ts, wotlk-item-names.ts, wotlk-item-gear-slots.ts, wotlk-item-equip-props.ts, wotlk-item-drop-sources.json (+ other bundled JSON)
├── utils/            # validate-character/dungeon, dungeon-toggles, character-display, format-character-details, format-character-export, build-export-status, format-dungeon-label, dungeon-type.ts, dungeon-short-name, bis-lists.ts, item-drop-sources.ts, gear-upgrade-hint.ts, character-gear-hints.ts, item-equip-restrictions.ts, tier-set-hint.ts, parse-wowsims-exporter.ts, format-stored-gear.ts, hide-external-wow-tooltips.ts, sort/filter, …
