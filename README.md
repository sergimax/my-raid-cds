...
9. **Edit character** — **Edit** icon in a character column header opens a dialog to update main/off spec and gear score (name and class stay fixed). Class and spec fields show WoW icons; WowSimsExporter import section headers use the spec icon when a spec is set. Separate import sections for **main** and **off** spec gear (off section appears when off spec is set). Changing a spec clears that section’s gear until you import again for the new spec. Imported items show as colored, underlined item links (ilvl-tier color) beside muted slot and ilvl labels, with external tooltips on hover.
