...
10. **Gear upgrade hints** — When a character has imported gear for a spec, toggle cells may show a background tint for upgrades in that raid (brighter = more slots). **Amber** tint = missing **BiS list** targets (exact or normal variant) that drop here when a list is selected for that spec. **Muted gray** tint = other spec-relevant ilvl upgrades (tooltip only; no ilvl rainbow on dungeon rows). Hints are **per spec** (main and off each use their own gear and BiS list); dual-spec cells split left (main) / right (off). Hover for per-spec summaries — BiS loot lists (exact + normal variants) and generic ilvl upgrades are separate sections; tier set tokens on ICC / ToC token rows when applicable. Custom dungeons without bundled loot use peak ilvl only. Boss and raid labels follow the UI locale (EN/RU). See [docs/roadmap.md](docs/roadmap.md) for possible follow-ups (trinket procs, finer tuning).
