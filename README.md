...
- **Table** — Sort by name, type (size + Heroic), ilvl, or completions; mixed dungeon search by raid name, size, and mode (e.g. `ICC`, `Uld10`, `ICC25N` / `ЦЛК25об`, `ToC25H` / `ИК25хм`; EN/RU names work in either locale); compact layout on narrow screens.
...
- **Gear pick** — Plan soft rolls for one character + spec. Open via toolbar **Gear pick** (EN) / **Подбор шмота** (RU). Set raid soft rules (max softs 1–4; **+100** or **re-roll**), pick a single main/off spec, then assign your softs and competing calls (histogram by soft weight) on BiS / BiS-variant upgrades from table-filtered raids. Copy a soft-call summary for the raid. Session-only (not persisted).
...
Toolbar panels (character pick, gear pick, BiS builds, add character, add dungeon) are mutually exclusive and share the same outlined panel shell.
