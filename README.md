...
8. **BiS lists** — **BiS lists** in the toolbar opens a panel to pick class and spec, browse built-in presets (read-only; **Titans** lists for 21 WotLK specs plus **Default** Blood/Unholy Death Knight lists), and create or edit local presets with item IDs per gear slot. Specs without a built-in list (e.g. Warrior Arms) still get the full slot editor and **Save list** form so you can build a custom list from scratch. Item names in slot rows use the same colored link styling as stored gear. Confirm each slot with the check icon; **Save list** stores a local preset under a custom name (built-in lists can be copied this way). Delete local presets from the preset chips. Slot validation rejects items the selected class/spec cannot equip (armor type, weapons, ranged slot). The selected preset for each class/spec drives upgrade-hint filtering for characters with that spec.
...
scripts/              # build-wow-data.mjs, bis-preset-format.mjs, comment-bis-presets.mjs, titans-bis/ (generate + parse + source.md), wow-item-gear-slots.mjs, wowsims-db.json (WowSims item DB for data build)
```
