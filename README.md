...
8. **BiS lists** — **BiS lists** in the toolbar opens a panel to pick class and spec, browse built-in presets (read-only; includes Blood and Unholy Death Knight defaults), and create or edit local presets with item IDs per gear slot. Item names in slot rows use the same colored link styling as stored gear. Confirm each slot with the check icon; **Save list** stores a local preset under a custom name (built-in lists can be copied this way). Delete local presets from the preset chips. Slot validation rejects items the selected class/spec cannot equip (armor type, weapons, ranged slot). The selected preset for each class/spec drives upgrade-hint filtering for characters with that spec.
