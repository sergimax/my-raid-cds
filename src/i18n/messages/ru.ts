...
export const ruMessages: Messages = {
  ...
  exportPanel: {
    ...
    instructions:
      "Отфильтруйте подземелья поиском в таблице, скопируйте строки ниже — по одной на рейд с персонажами без КД (переключатель выкл.). Отметьте спеки для каждого персонажа. Можно включить ползунок минимального GS (напр. 5.6) — спеки ниже порога не попадут в экспорт.",
    ...
    minGearScoreHelper:
      "В экспорт попадут только отмеченные спеки с GS не ниже выбранного значения.",
