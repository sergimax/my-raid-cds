...
export const ruMessages: Messages = {
  ...
  exportPanel: {
    ...
    instructions:
      "Отфильтруйте подземелья поиском в таблице, скопируйте строки ниже — по одной на рейд с персонажами без КД (переключатель выкл.). Отметьте спеки для каждого персонажа. Можно задать минимальный GS — спеки ниже порога не попадут в экспорт.",
    noCharacters: "Добавьте персонажа для формирования сводки.",
    exportText: "Текст экспорта",
    minGearScore: "Мин. GS",
    minGearScoreHelper: "В экспорт попадут только отмеченные спеки с GS не ниже порога. Пусто — без фильтра.",
    minGearScoreInvalid: "Введите целое число от {{min}} до {{max}} или оставьте пустым.",
