...
export const ruMessages: Messages = {
  ...
  exportPanel: {
    ...
    instructions:
      "Отфильтруйте подземелья поиском в таблице, скопируйте строки ниже — по одной на рейд с персонажами без КД (переключатель выкл.). Отметьте спеки для каждого персонажа. Можно задать минимальный GS (напр. 5.6 или 5600) — спеки ниже порога не попадут в экспорт.",
    noCharacters: "Добавьте персонажа для формирования сводки.",
    exportText: "Текст экспорта",
    minGearScore: "Мин. GS",
    minGearScoreHelper: "Краткий GS вроде 5.6 или полное значение 5600. В экспорт попадут отмеченные спеки с GS не ниже порога. Пусто — без фильтра.",
    minGearScoreInvalid: "Введите краткий GS (напр. 5.6), полное значение ({{min}}–{{max}}) или оставьте пустым.",
